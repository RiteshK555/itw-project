...
def index(request,id):
    ...
    buyer=money.objects.get(holder=request.user)
    seller=money.objects.get(holder=a_product.seller)
    initial_buyer_money=buyer.amount
    initial_seller_money=seller.amount
    buyer.amount=initial_buyer_money-a_product.price
    seller.amount=initial_seller_money+a_product.price
    buyer.save()
    seller.save()
